...
*/
		...
		if (Temperature < 0) {
			...
			for (int i=0; i<neighbours.Count; ++i) {
				if (neighbours [i].Temperature < Temperature) {
					float fNewTemp = neighbours [i].Temperature + (Temperature - neighbours [i].Temperature) * (neighbours [i].GetSize () / fSumSize) * 0.2f;
	...
	public void Init (BreakableContainer tContr, Transform tTransform = null, Vector3 tTranslate = new Vector3 ())
	{
		...
		if (tTransform != null) {
			...
			if (Damage > Container.FractureForce) {
				if (transform.localScale.x <= BoxPool.DebrisSize && transform.localScale.y <= BoxPool.DebrisSize) {
	...
	public void Break ()
	{
		...
		if ((iBreakX * iBreakY - 1) <= BoxPool.Instance.GetFreePoolSize ()) {
			...
			Damage -= Container.FractureForce / GetSize ();
			if (Damage < 0)
				Damage = 0;
			else
				...
				for (int y = 0; y < iBreakY; ++y) {
					if (x == 0 && y == 0) {
						...
						if (tContr != null) {
							...
							tContr.OriginalColor = Sprite.color;
						}
					}
				}
			}
