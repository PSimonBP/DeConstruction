...
public class LegController : MonoBehaviour
{
	...
	void Update ()
	{
		...
		switch (tMovement) {
		case UnitController.EMovementStates.MS_WALK_FWD:
			fWalkSpeed = 1.0f;
			break;
		case UnitController.EMovementStates.MS_WALK_FWD_L:
			fWalkSpeed = 1.0f;
			fTurnSpeed = 10.0f;
			break;
		case UnitController.EMovementStates.MS_WALK_FWD_R:
			fWalkSpeed = 1.0f;
			fTurnSpeed = -10.0f;
			break;
		case UnitController.EMovementStates.MS_RUN_FWD:
			fWalkSpeed = 1.6f;
			break;
		case UnitController.EMovementStates.MS_RUN_FWD_L:
			fWalkSpeed = 1.6f;
			fTurnSpeed = 10.0f;
			break;
		case UnitController.EMovementStates.MS_RUN_FWD_R:
			fWalkSpeed = 1.6f;
			fTurnSpeed = -10.0f;
			break;
		case UnitController.EMovementStates.MS_WALK_BWD:
			fWalkSpeed = -0.6f;
			break;
		case UnitController.EMovementStates.MS_WALK_BWD_L:
			fWalkSpeed = -0.6f;
			fTurnSpeed = 10.0f;
			break;
		case UnitController.EMovementStates.MS_WALK_BWD_R:
			fWalkSpeed = -0.6f;
			fTurnSpeed = -10.0f;
			break;
		case UnitController.EMovementStates.MS_WALK_LEFT:
			fSideSpeed = -0.6f;
			break;
		case UnitController.EMovementStates.MS_WALK_RIGHT:
			fSideSpeed = 0.6f;
			break;
		case UnitController.EMovementStates.MS_TURN_L:
			fTurnSpeed = 10.0f;
			break;
		case UnitController.EMovementStates.MS_TURN_R:
			fTurnSpeed = -10.0f;
			break;
		}
...
//		m_tController.rigidbody2D.AddRelativeForce(m_tMoveSpeed * 3, ForceMode2D.Force);
		Rigidbody2D tBody = m_tController.GetComponent<Rigidbody2D> ();
		tBody.velocity = tBody.GetRelativeVector (m_tMoveSpeed) * tBody.transform.localScale.x * 0.75f;
		tBody.angularVelocity = m_fTurnSpeed * 6;
	}
